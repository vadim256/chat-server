...
SaveInDB::SaveInDB(std::string const & name_database){
    ...
    std::string request_sql = "SELECT name FROM sqlite_master WHERE type='table' AND name='list_messages';";
...
void SaveInDB::SetMessage(wxString const & msg){
    m_message = msg;
}

wxString SaveInDB::GetMessage() const {
    return m_message;
}
...
void SaveInDB::WriteMessageInDB(){
    ...
    result = sqlite3_bind_text(stmt, index, (const char*)m_message.mb_str(wxConvUTF8),
                               m_message.Len()+1, SQLITE_TRANSIENT);
    assert(result == SQLITE_OK);
    result = sqlite3_step(stmt);
    assert(result == SQLITE_DONE);
    sqlite3_finalize(stmt);
...
void SaveInDB::ReadMessageInDB(){
    ...
    while((result = sqlite3_step(stmt)) == SQLITE_ROW){
        m_allmessages.push_back(wxString((const char*)sqlite3_column_text(stmt,0)));
    }
    assert(result == SQLITE_DONE);
    sqlite3_finalize(stmt);
}
